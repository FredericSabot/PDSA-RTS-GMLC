...
class OperatingPoint:
    def __init__(self, network: pp.network.Network):
        ...
        loads = network.get_loads().fillna(0)

        production_by_source = gens.groupby("energy_source")["p"].sum()
        self.total_wind = -production_by_source.get("WIND", 0)
        self.total_solar = -production_by_source.get("SOLAR", 0)
        self.total_load = loads["p0"].sum()
    ...
    def flatten(self: OperatingPoint) -> np.ndarray:
        return np.concatenate([[self.total_wind, self.total_solar, self.total_load], self.gens_p, self.gens_q, self.lines_p, self.lines_q])
